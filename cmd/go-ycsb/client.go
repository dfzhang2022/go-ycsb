...
func runClientCommandFunc(cmd *cobra.Command, args []string, doTransactions bool, doCollectingGeneratedData bool, command string) {
	dbName := args[0]
	fmt.Println("Now loading data using DB: ", dbName)
	initialGlobal(dbName, func() {
		doTransFlag := "true"
		doCollectingFlag := "true" // Flags whether the build data needs to be collected
		if !doTransactions {
			doTransFlag = "false"
		}
		if !doCollectingGeneratedData {
			doCollectingFlag = "false"
		}
		globalProps.Set(prop.DoTransactions, doTransFlag)
		globalProps.Set(prop.DoCollectingData, doCollectingFlag)
...
func runLoadCommandFunc(cmd *cobra.Command, args []string) {
	runClientCommandFunc(cmd, args, false, false, "load")
}

func runTransCommandFunc(cmd *cobra.Command, args []string) {
	runClientCommandFunc(cmd, args, true, false, "run")
}

func runCollectingDataCommandFunc(cmd *cobra.Command, args []string) {
	runClientCommandFunc(cmd, args, false, true, "collect")
}
...
func newCollectDataCommand() *cobra.Command {
	m := &cobra.Command{
		Use:   "collect generated data",
		Short: "collect YCSB  benchmark, then redirect to files",
		Args:  cobra.MinimumNArgs(1),
		Run:   runCollectingDataCommandFunc,
	}
	initClientCommand(m)
	return m
}
