...
func (c *core) DoOutputDataSet(ctx context.Context, p *properties.Properties, opCount int64) error {
	...
	var dataset_map map[string]string = make(map[string]string)
	...
	for i < int(opCount) {
		...
		RowKey := fmt.Sprintf("%s:%s", table_name, dbKey)
		data, err := json.Marshal(values)
		if err != nil {
			return err
		}
		dataset_map[RowKey] = string(data)
		// fmt.Println("Key=", RowKey, " Value=", string(data))
		i++
...
func (c *core) DoOutputOperationSet(ctx context.Context, p *properties.Properties, opCount int64) error {
	...
	operation_set_arr := make([]OperationData, 0)
	...
	for i < int(opCount) {
		...
		case read:
			// fmt.Println("read")
			// fmt.Println("Read: ", keyName, fields)
			// fmt.Println("Read: ", keyName)
			var tmp_operation OperationData
			tmp_operation.OperType = operation
			tmp_operation.KeyName = keyName
			tmp_operation.Value = ""
			// fmt.Println(tmp_operation)
			operation_set_arr = append(operation_set_arr, tmp_operation)
			// return c.doTransactionRead(ctx, db, state)
		case update:
			values = c.buildValues(state, keyName)
			rkey := fmt.Sprintf("%s:%s", c.table, keyName)
			data, err := json.Marshal(values)
			if err != nil {
				return err
			}
			// fmt.Println("Update: ", rkey, string(data))
			var tmp_operation OperationData
			tmp_operation.OperType = operation
			tmp_operation.KeyName = rkey
			tmp_operation.Value = string(data)
			// fmt.Println(tmp_operation)
			operation_set_arr = append(operation_set_arr, tmp_operation)
			// fmt.Println("Update: ", keyName, values)
			// fmt.Println("update")
	...
	var err error
	// 通过 JSON 序列化字典数据
	// fmt.Println(len(operation_set_arr))
	// fmt.Println(operation_set_arr)

	output := make([]string, 0)
	for idx := 0; idx < len(operation_set_arr); idx++ {
		// var tmp_unit_in_json []byte
		// var tmp_operation OperationData = operation_set_arr[idx]
		tmp_unit_in_json, _ := json.Marshal(operation_set_arr[idx])
		// fmt.Println(string(tmp_unit_in_json))
