...
func newWorker(p *properties.Properties, threadID int, threadCount int, workload ycsb.Workload, db ycsb.DB) *worker {
	w := new(worker)
	w.p = p
	w.doTransactions = p.GetBool(prop.DoTransactions, true) // 标记进入到负载的运行
	w.doCollectingData = p.GetBool(prop.DoCollectingData, true)
	...
	w.recordCount = p.GetInt64(prop.RecordCount, 0)
	w.operationCount = p.GetInt64(prop.OperationCount, 0)
...
func (w *worker) run(ctx context.Context) {
	...
	startTime := time.Now()
	// fmt.Println("w.opCount = ", w.opCount)
	// w.workload.DoOutputDataSet(ctx, w.workDB, w.opCount)
		...
		for w.opCount == 0 || w.opsDone < w.opCount {
			...
			if w.doTransactions {
				if w.doBatch {
					...
					err = w.workload.DoTransaction(ctx, w.workDB) // 负载部分的内容生成
				}
			} else {
				// w.workload.DoOutputDataSet(ctx, w.workDB, w.opCount)
				if w.doBatch {
